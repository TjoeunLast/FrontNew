...
export function ShipperHomeScreen() {
  ...
  // 페이지 이동 함수들
  const goCreateOrder = () =>
    router.push("/(shipper)/create-order/step1-route" as any);
  const goChat = () => router.push("/(chat)" as any);
  const goNotificationsTab = () =>
    router.push("/(shipper)/(tabs)/notifications" as any);
  const goDispatchTab = (targetTab: "WAITING" | "PROGRESS" | "DONE") => {
    router.push({
      pathname: "/(shipper)/(tabs)/orders",
  ...
  const summary: SummaryItem[] = useMemo(() => {
    ...
    const matching = liveOrders.filter(
      (x) => x.status === "MATCHING" || x.status === "DISPATCHED",
    ).length;
  ...
  const hasApplicantRequest = useMemo(
    () =>
      liveOrders.some(
        (x) => x.status === "MATCHING" && (x.applicantsCount ?? 0) > 0,
      ),
    [liveOrders],
  );
    ...
    <View style={[s.page, { backgroundColor: c.bg.canvas }]}>
      ...
      >
        <View style={s.dashboardContainer}>
          <Text style={[s.dashboardTitle, { color: c.text.primary }]}>
            운송 현황
          </Text>
          <View style={s.summaryRow}>
            {summary.map((it) => {
              ...
              return (
                ...
                >
                  <View
                    style={[s.summaryIconCircle, { backgroundColor: iconBg }]}
                  >
                    {it.key === "driving" ? (
                      ...
                      <Ionicons
                        name={
                          it.key === "matching" ? "cube" : "checkmark-circle"
                        }
                        size={20}
                        color={iconColor}
                      />
                    )}
                    {it.key === "matching" && hasApplicantRequest ? (
                      <View style={s.redDot} />
                    ) : null}
                  </View>
                  <Text style={[s.summaryLabel, { color: c.text.secondary }]}>
                    {it.label}
                  </Text>
          ...
          <View style={{ gap: 12 }}>
            {recentOrders.map((item) => (
              <RecommendedOrderCard
                ...
                statusLabel={
                  item.status === "DRIVING" &&
                  isWithinNextHour(item.dropoffTimeHHmm)
                    ? "곧 도착"
...
const s = StyleSheet.create({
  ...
  dashboardTitle: { fontSize: 18, fontWeight: "700", marginBottom: 16 },
  ...
  sectionTitle: { fontSize: 18, fontWeight: "700", lineHeight: 19 },
